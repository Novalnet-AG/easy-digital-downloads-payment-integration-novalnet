 ...
 *
 * This gateway is used for real time processing of Credit Card Payment.
 *
...
class Novalnet_Cc {
    ...
    public function novalnet_cc_script_functions() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'card_expiry_input'  => __( 'MM / YYYY', 'edd-novalnet' ),
                'card_cvc_label'     => __( 'CVC/CVV/CID', 'edd-novalnet' ),
                'card_cvc_input'     => __( 'XXX', 'edd-novalnet' ),
                'card_cvc_hint'      => __( 'what is this?', 'edd-novalnet' ),
    ...
    public function register_novalnet_cc_gateway( $gateway_sections ) {
        if ( edd_is_gateway_active( 'novalnet_cc' ) ) {
            $gateway_sections['novalnet_cc'] = __( 'Novalnet Credit Card', 'edd-novalnet' );
        }
    ...
    public function register_novalnet_cc( $gateways ) {
        ...
        $novalnet_cc = array(
            'novalnet_cc' => array(
                'admin_label'    => __( 'Novalnet Credit Card', 'edd-novalnet' ),
                'checkout_label' => __( 'Credit Card', 'edd-novalnet' ),
                'supports'       => array(),
            ),
        );
    ...
    public function display_form() {
        ...
        $payment_method_name = __( 'Credit Card', 'edd-novalnet' );
        $serverip            = novalnet_server_addr( 'SERVER_ADDR' );
        $vendor              = $edd_options['novalnet_merchant_id'];
        $product             = $edd_options['novalnet_product_id'];
        ...
        <fieldset>
            ...
            <p>
                <label for="novalnet_cc_payment_desc">
                    <span class="edd-description" style="font-size:90%;">
                        <?php echo esc_html( __( 'You will be redirected to Credit Card. Please don’t close or refresh the browser until the payment is completed', 'edd-novalnet' ) ); ?>
                    </span>
                </label>
            </p>
            <p>
            ...
            <p>
                ...
                ?>
        <iframe id="nnIframe" width="70%" frameborder="0" scrolling="no" onload="novalnet_load_iframe()" src="<?php echo $path; ?>"></iframe>
    ...
    public function novalnet_cc_process_payment( $purchase_data ) {
        ...
        $params ['cc_3d'] = 1; // Assign CC3D parameter if cc3d enabled.
    ...
    public function register_novalnet_cc_settings( $gateway_settings ) {
        ...
        $novalnet_cc                     = array(
            array(
                ...
                'name' => '<strong><font color="#1874CD">' . __( 'Novalnet Credit Card', 'edd-novalnet' ) . '</font> </strong>',
            ...
            array(
                'id'            => 'novalnet_cc_mastero_accept',
                'name'          => __( 'Display Maestro logo', 'edd-novalnet' ),
                'type'          => 'checkbox',
                'tooltip_title' => __( 'Display Maestro logo', 'edd-novalnet' ),
                'tooltip_desc'  => __( 'Display Maestro logo at the checkout page', 'edd-novalnet' ),
                
            ),
            array(
                'id'            => 'novalnet_cc_amex_accept',
                'name'          => __( 'Display AMEX logo', 'edd-novalnet' ),
                'type'          => 'checkbox',
                'tooltip_title' => __( 'Display AMEX logo', 'edd-novalnet' ),
                'tooltip_desc'  => __( 'Display AMEX logo at the checkout page', 'edd-novalnet' ),
            ),
            array(
