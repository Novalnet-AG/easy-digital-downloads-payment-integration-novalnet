...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    // To add novalnet admin portal in global configuration.
    add_action( 'admin_menu', 'novalnet_admin_menus', 9 );
...
function novalnet_signup_args( $args, $item ) {
    ...
    if ( ! empty( $params ) && novalnet_check_string( $item->purchase_data['gateway'] ) ) {
        ...
        if ( ( in_array($item->purchase_data['gateway'], array('novalnet_cc', 'novalnet_paypal'), true ) ) ) {
...
function novalnet_handle_api( $data ) {
    switch ( $data['edd-api'] ) {
        ...
        case 'novalnet_update_payment_method':
            if ( 'novalnet_redirect_response' === $data['edd-api'] ) {
...
/**
 * Adds separate sub-menu for Novalnet Admin Portal under Downloads EDD
 */
function novalnet_admin_menus() {
        add_submenu_page(
            'edit.php?post_type=download',
            __( 'Novalnet Updates', 'edd-novalnet' ),
            __( 'Novalnet Updates', 'edd-novalnet' ),
            'manage_shop_settings',
            'edd-novalnet-update',
            'novalnet_admin_infor'
        );
}
 ...
 * @since 1.1.1
 */
function novalnet_admin_infor() {
    if ( ! current_user_can( 'administrator' ) ) {
        return;
    }
    include_once NOVALNET_PLUGIN_DIR . 'includes/admin/updates/novalnet-updates.php';
}
...
function novalnet_update_action() {
    ...
    $show_update_page   = get_option( 'show_novalnet_update_page' );
    if ( ! empty( $current_db_version ) && version_compare( $current_db_version, NOVALNET_VERSION, '!=' ) ) {
        ...
        delete_option( 'show_novalnet_update_page' );
        wp_safe_redirect( admin_url( 'edit.php?post_type=download&page=edd-novalnet-update' ) );
        exit();
    }
}
    ...
    if ( $icon_html ) {
        ...
        if ( 'novalnet_cc' === $payment_method ) {
            ...
            foreach ( $cc_logos as $value ) {
                if ( $value['active'] ) {
                    ?>
                    <img class="payment-icon" title="<?php echo esc_html( $payment_method_name ); ?>" alt="<?php echo esc_html( $payment_method_name ); ?>" style="float: right;" src="<?php echo esc_html( NOVALNET_PLUGIN_URL ) . 'assets/images/' . esc_html( $value['logo'] ) . '.png'; ?>">
                    <?php
                }
            }
        }
        if ( 'novalnet_cc' === $payment_method ) {
            ?>
            <img class="payment-icon" title="<?php echo esc_html( $payment_method_name ); ?>" alt="<?php echo esc_html( $payment_method_name ); ?>" style="float:right;" src="<?php echo esc_html( NOVALNET_PLUGIN_URL . 'assets/images/' . $payment_method . '_master.png' ); ?>">
               
            <img class="payment-icon" title="<?php echo esc_html( $payment_method_name ); ?>" alt="<?php echo esc_html( $payment_method_name ); ?>" style="float:right;" src="<?php echo esc_html( NOVALNET_PLUGIN_URL . 'assets/images/' . $payment_method . '_visa.png' ); ?>">
        <?php } ?>
        <?php
    }
}
...
function novalnet_get_cc_logos() {
    ...
    $cc_logos = array(
        'maestro'   => array(
            'logo'   => 'novalnet_cc_maestro',
            'active' => isset( $edd_options['novalnet_cc_mastero_accept'] ) ? $edd_options['novalnet_cc_mastero_accept'] : '',
        ),
        'amex_card' => array(
            'logo'   => 'novalnet_cc_amex',
            'active' => isset( $edd_options['novalnet_cc_amex_accept'] ) ? $edd_options['novalnet_cc_amex_accept'] : '',
        ),
    );
    return $cc_logos;
}
...
function remove_novalnet_payment( $gateway_list ) {
    ...
    if ( $recurring && isset( $edd_options['novalnet_subs_payments'] ) ) {
        ...
        if ( novalnet_check_string( $edd_options['default_gateway'] ) && isset( $edd_options['default_gateway'] ) && ! in_array( $edd_options['default_gateway'], $edd_options['novalnet_subs_payments'], true ) ) {
        ...
        } else if( ! in_array( $edd_options['default_gateway'], array_keys( $gateway_list ), true ) ){
...
function novalnet_get_redirect_param( $payment_name, $payment_data = array(), $subs_update = array() ) {
    ...
    $encode_array['auth_code'] = $edd_options['novalnet_auth_code'];
    $encode_array['product']   = $edd_options['novalnet_product_id'];
    $encode_array['tariff']    = $payment_data['tariff'];
    $encode_array['test_mode'] = ( ( edd_is_test_mode() ) ? 1 : ( isset( $edd_options[ $payment_name . '_test_mode' ] ) ? 1 : 0 ) );
